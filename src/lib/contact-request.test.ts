...
describe("contact request boundary", () => {
  it("reads valid JSON and rejects malformed or missing bodies", async () => {
    await expect(readContactBody(request('{"hello":"world"}'))).resolves.toEqual({
      ok: true,
      value: { hello: "world" },
    });
    ...
    await expect(readContactBody(new Request("https://example.com"))).resolves.toEqual({
      ok: false,
      status: 400,
    });
  });
  it("caps actual bytes even when Content-Length is omitted or forged", async () => {
    for (const headers of [{}, { "content-length": "2" }, { "content-length": "17000" }])
      await expect(readContactBody(request("x".repeat(17000), headers))).resolves.toEqual(
        { ok: false, status: 413 },
      );
  });
